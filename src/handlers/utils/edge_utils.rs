use sqlx::PgPool;
use uuid::Uuid;
use crate::models::edge::{NewEdgeRequest, Edge};
...
pub async fn fetch_edges_by_source_and_target_db(pool: &PgPool, graph_id: Uuid, source_id: Uuid, target_id: Uuid) -> Result<Vec<Edge>, sqlx::Error> {
    let edges = sqlx::query_as!(
        Edge,
...
pub async fn fetch_edges_by_source_db(pool: &PgPool,graph_id: Uuid, source_id: Uuid, ) -> Result<Vec<Edge>, sqlx::Error> {
    let edges = sqlx::query_as!(
        Edge,
...
pub async fn fetch_edges_by_target_db(pool: &PgPool, graph_id: Uuid, target_id: Uuid) -> Result<Vec<Edge>, sqlx::Error> {
    let edges = sqlx::query_as!(
        Edge,
