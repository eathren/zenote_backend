...
mod node_handler_tests {
    ...
    use crate::{
        handlers::{
            ...
            utils::{
                ...
                node_utils::{
                    create_node_db, delete_node_db, fetch_all_nodes_db, fetch_node_db,
                    update_node_db,
                },
                user_utils::create_user_db,
            },
        },
        models::node::{NewNodeRequest, UpdateNodeRequest},
    };
    use sqlx::PgPool;
    use uuid::Uuid;

    async fn setup_user_and_graph(pool: &PgPool) -> (String, Uuid) {
    ...
    async fn test_create_and_fetch_node() {
        ...
        let created_node = create_node_db(&pool, node_options)
            .await
            .expect("Failed to create node");

        assert_eq!(created_node.name, node_name, "Node name should match");

        let fetched_node = fetch_node_db(&pool, created_node.id)
            .await
            .expect("Failed to fetch node");

        assert_eq!(
            fetched_node.id, created_node.id,
            "Fetched node should have the same ID"
        );
    }
    ...
    async fn test_update_node() {
        ...
        update_node_db(&pool, created_node.id, update_request)
            .await
            .expect("Failed to update node");

        let updated_node = fetch_node_db(&pool, created_node.id)
            .await
            .expect("Failed to fetch updated node");

        assert_eq!(
            updated_node.name, updated_name,
    ...
    async fn test_delete_node() {
        ...
        delete_node_db(&pool, created_node.id)
            .await
    ...
    async fn test_fetch_all_nodes_for_graph() {
        ...
        create_node_db(&pool, node_options_1)
            .await
            .expect("Failed to create first node");
        create_node_db(&pool, node_options_2)
            .await
            .expect("Failed to create second node");

        let nodes = fetch_all_nodes_db(&pool, graph_id)
            .await
            .expect("Failed to fetch nodes for graph");

        assert!(
            nodes.len() >= 2,
            "Should fetch at least two nodes for the graph"
        );
    }
}
