...
async fn test_create_user_db() {
    ...
    let result = create_user_db(&pool, user_id, unique_email).await;
    assert!(result.is_ok());
}
...
async fn test_create_same_user_db() {
    ...
    let first_attempt = create_user_db(&pool, user_id.clone(), email.clone()).await;
...
async fn test_delete_user_db() {
    ...
    let user = create_user_db(&pool, user_id, email).await.unwrap();
